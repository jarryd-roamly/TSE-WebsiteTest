'use client';
// app/admin/region-video/page.tsx — v3
// Fixed: Save button always active when URL field has content
// Fixed: Speed toggle uses rateRef so closures never go stale
// Fixed: Interval polling ensures rate applies even on slow-loading videos
...
const toJourneySlug = (s: string) => `${s}-journey`;
...
function VideoCard({ slug, label, country, accent, tagline, videoUrl, status, err, onUpload, onRemove, onSaveUrl }: {
  ...
  onSaveUrl: (slug: string, url: string) => Promise<void>;
}) {
  ...
  // rateRef — always holds the live value so video event callbacks never go stale
  const rateRef = useRef(1);
  const vidRef  = useRef<HTMLVideoElement>(null);

  // Sync URL input when videoUrl prop changes (after upload)
  useEffect(() => { setUrlInput(videoUrl); }, [videoUrl]);

  // Apply rate to video element — uses rateRef, not state, so always current
  const applyRate = () => {
    if (vidRef.current) vidRef.current.playbackRate = rateRef.current;
  };

  // Poll every 800ms — catches slow-loading videos and any browser reset
  useEffect(() => {
    const id = setInterval(applyRate, 800);
  ...
  const handleSetRate = (r: number) => {
    ...
    setRate(r);
    applyRate();
    setTimeout(applyRate, 150);
    setTimeout(applyRate, 500);
  };
  ...
  // Save button is active whenever the field has any content
  const saveActive = !!urlInput.trim() && !saving;
  ...
  return (
    <div style={{ background: T.surface2, border: `0.5px solid ${videoUrl ? 'rgba(212,175,55,0.2)' : T.border}`, borderRadius: 12, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '12px 13px' }}>
        ...
        {/* Row 1: Upload + Speed pair + Remove */}
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' as const, alignItems: 'center', marginBottom: 10 }}>
          ...
          {/* Speed: two joined buttons — highlighted = active */}
          {videoUrl && (
            <div style={{ display: 'flex', borderRadius: 7, overflow: 'hidden', border: `0.5px solid rgba(255,255,255,0.12)`, flexShrink: 0 }}>
              <button
                ...
                style={{ padding: '7px 16px', border: 'none', borderRight: `0.5px solid rgba(255,255,255,0.12)`, background: rate === 1 ? T.goldDim : 'rgba(255,255,255,0.03)', color: rate === 1 ? T.gold : T.textMid, fontSize: 11, fontWeight: rate === 1 ? 600 : 300, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s' }}
              >1×</button>
              <button
                onClick={() => handleSetRate(0.5)}
                style={{ padding: '7px 16px', border: 'none', background: rate === 0.5 ? T.goldDim : 'rgba(255,255,255,0.03)', color: rate === 0.5 ? T.gold : T.textMid, fontSize: 11, fontWeight: rate === 0.5 ? 600 : 300, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s' }}
              >½×</button>
            </div>
          )}
        ...
        {/* Row 2: URL field + Save — Save is active whenever field has content */}
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          <input
            value={urlInput}
            onChange={e => { setUrlInput(e.target.value); setSaveMsg(''); }}
            placeholder="Paste video URL and click Save"
            style={{ flex: 1, background: 'rgba(255,255,255,0.03)', border: `0.5px solid rgba(255,255,255,0.1)`, color: T.text, borderRadius: 6, padding: '7px 10px', fontSize: 10, fontFamily: 'inherit', outline: 'none', minWidth: 0 }}
          />
          <button
            onClick={handleSaveUrl}
            disabled={!saveActive}
            style={{
              background:    saveActive ? T.goldDim              : 'rgba(255,255,255,0.02)',
              border:        `0.5px solid ${saveActive ? T.borderGold : 'rgba(255,255,255,0.07)'}`,
              color:         saveActive ? T.gold                 : 'rgba(245,240,232,0.2)',
              borderRadius:  6,
              padding:       '7px 16px',
              cursor:        saveActive ? 'pointer'              : 'default',
              fontSize:      11,
              fontWeight:    500,
              fontFamily:    'inherit',
        ...
        {/* Status messages */}
        {status === 'uploading' && <div style={{ marginTop: 8, fontSize: 10, color: T.amber }}>⏳ Uploading to R2…</div>}
        ...
        {err && !saveMsg        && <div style={{ marginTop: 6, fontSize: 10, color: T.red }}>{err}</div>}
      </div>
    </div>
  );
}
      ...
      <div style={{ maxWidth: 1020, margin: '0 auto', padding: '32px 20px 80px' }}>
        ...
        <div style={{ marginBottom: 44 }}>
          ...
          <div style={{ fontSize: 13, color: T.textDim, lineHeight: 1.7, maxWidth: 600 }}>
            Two separate video sets. Upload an MP4 file, or paste an external URL into the field and click <strong style={{ color: T.text, fontWeight: 400 }}>Save</strong> — the button lights up as soon as the field has content.
          </div>
          <div style={{ marginTop: 14, padding: '10px 16px', background: T.goldDim, border: `0.5px solid ${T.borderGold}`, borderRadius: 8, fontSize: 12, color: T.gold, lineHeight: 1.65 }}>
            ✦ &nbsp;<strong>1×</strong> = normal speed &nbsp;·&nbsp; <strong>½×</strong> = half speed. The highlighted button is the active speed. Speed applies immediately and self-corrects every 800ms.
          </div>
        </div>
          ...
          <>
            <div style={{ marginBottom: 60 }}>
              <SectionHead
                ...
                sub="Plays beside the form as the traveller selects a destination. Wide landscape clips work best."
                color={T.gold}
              />
            ...
            <div>
              <SectionHead
                ...
                sub="Plays full-screen while the AI builds the itinerary. Reads these first — falls back to the Destination Panel clip only if no Cinematic clip is uploaded for that region."
                color="#60a5fa"
              />
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(285px,1fr))', gap: 14 }}>
                {REGIONS.map(r => {
