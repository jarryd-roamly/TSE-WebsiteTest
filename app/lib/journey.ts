// app/lib/journey.ts — v3.0
// Enriched types: Segment.images, Segment.inclusions, Journey.activities/occasion/themes
// Fixed: compToLeg constructs valid ISO datetimes; NaN-proof duration from total_duration_minutes
...
export type Leg = {
  ...
  durationMin?: number;        // NEW: minutes, avoids NaN from bad ISO strings
...
export type Segment = {
  ...
  inclusions: string[];        // NEW: rate_includes list
  images: string[];            // NEW: all property images for carousel
  ...
  nights?: number;
  regionSlug?: string;         // NEW: for activity matching
};
...
export type Journey = {
  ...
  transfers: Transfer[];
  homeward: Transfer;
  activities?: ActivityItem[];   // NEW: all selected activities
  occasion?: string;             // NEW: 'honeymoon' | 'anniversary' | 'family' | 'birthday' | ''
  themes?: string[];             // NEW: ['predators','water','culture',...]
};

// ── helpers ───────────────────────────────────────────────────────────────────
function formatDateRange(checkIn: string, checkOut: string): string {
  if (!checkIn) return 'Dates TBC';
  const a = new Date(checkIn); const b = checkOut ? new Date(checkOut) : null;
...
function regionLabel(slug: string): string {
  const map: Record<string, string> = {
    ...
    'chobe-vic-falls': 'Chobe / Victoria Falls', 'masai-mara': 'Masai Mara', 'bwindi': 'Bwindi',
  };
  return map[slug] || slug;
}

// ── Safe ISO datetime construction ────────────────────────────────────────────
// Duffel returns full ISO; our saved components may have split date+time fields.
// We always produce a full ISO or empty string — never a time-only string.
function safeISO(fullDT?: string, date?: string, time?: string): string {
  if (fullDT && fullDT.includes('T')) return fullDT;
  if (date && /^\d{4}-\d{2}-\d{2}$/.test(date)) {
    return `${date}T${time && /^\d{2}:\d{2}/.test(time) ? time.substring(0, 5) : '00:00'}:00`;
  }
  return '';
}
...
function mapToJourney(booking: any, itinerary: any): Journey {
  ...
  const segments: Segment[] = hotelComponents.map((comp: any, i: number) => {
    ...
    // Build images array from all available sources
    const images: string[] = [];
    if (comp.hero_image_url) images.push(comp.hero_image_url);
    if (Array.isArray(comp.images)) {
      comp.images.forEach((img: any) => {
        const url = typeof img === 'string' ? img : img?.url;
        if (url && url !== comp.hero_image_url) images.push(url);
      });
    }
  ...
  if (segments.length === 0) {
    segments.push({
      ...
      narrative: '', detail: [], inclusions: [], images: [], badges: [], acts: [], tone: '',
  ...
  const flyZAR = components.filter(c => (c.type || '').toLowerCase().includes('flight')).reduce((s, c) => s + (c.price_display_zar || 0), 0);
  const accZAR = totalZAR - flyZAR;

  // ── Build real Leg objects from saved components ──────────────────────────
  function compToLeg(comp: any): Leg | null {
    ...
    const depDT    = safeISO(comp.departure_datetime, comp.departure_date, comp.departure_time);
    const arrDT    = safeISO(comp.arrival_datetime, comp.departure_date, comp.arrival_time);
    const durMin   = comp.total_duration_minutes || comp.duration_minutes || 0;
    if (!fromApt || !toApt) return null;
    return {
      kind:        comp.is_international ? 'longhaul' : 'air',
      carrier:     airline,
      no:          flightNo,
      depApt:      fromApt,
      arrApt:      toApt,
  ...
  const arrivalTransfer: Transfer = {
    ...
    legs:  arrivalLegs,
    notes: arrivalLegs.length > 0 ? [] : [{ good: true, text: 'Airport arrival transfer confirmed by your specialist within 2 hours.' }],
  };
  ...
  const interRegionTransfers: Transfer[] = hotelComponents.length > 1
    ? hotelComponents.slice(0, -1).map((comp: any, i: number) => {
        ...
        return {
          ...
          legs,
          notes: legs.length > 0
            ? (xferComp?.description ? [{ good: true, text: xferComp.description }] : [])
  ...
  const homewardTransfer: Transfer = {
    ...
    legs:  homewardLegs,
    notes: homewardLegs.length > 0 ? [] : [{ good: true, text: 'Departure logistics confirmed by your specialist.' }],
  };

  // ── Activities ────────────────────────────────────────────────────────────
  const activities: ActivityItem[] = components
    .filter(c => c.pillar === 'activity' || c.type === 'activity')
    .map(c => ({ name: c.name, region: c.region_label || '', regionSlug: c.region_slug || '', duration: c.duration || '' }));

  // ── Occasion + themes detection ───────────────────────────────────────────
  const titleLower = (itinerary?.title || '').toLowerCase();
  const occasion = titleLower.includes('honey') ? 'honeymoon'
    : titleLower.includes('anniversary') ? 'anniversary'
    : titleLower.includes('family') ? 'family'
    : titleLower.includes('birthday') ? 'birthday'
    : snap.notes?.toLowerCase().includes('anniversary') ? 'anniversary'
    : '';
  const themes: string[] = [];
  if (route.some(r => /okavango|delta/.test(r.toLowerCase()))) themes.push('water & mokoro');
  if (route.some(r => /kruger|sabi|madikwe|phinda/.test(r.toLowerCase()))) themes.push('Big Five');
  if (route.some(r => /cape/.test(r.toLowerCase()))) themes.push('wine & culture');
  if (route.some(r => /victoria|falls|chobe/.test(r.toLowerCase()))) themes.push('adventure');
  if (route.some(r => /mara|masai|kenya/.test(r.toLowerCase()))) themes.push('migration');
  if (route.some(r => /bwindi|gorilla/.test(r.toLowerCase()))) themes.push('gorillas');
  ...
  return {
    ...
    specialist: {
      initials: 'SM', name: 'Sarah Mitchell', role: 'Senior Safari Specialist',
      rec: 'Your journey specialist — available on WhatsApp & email.', years: 8,
    },
    segments,
...
export async function getJourney(code: string): Promise<Journey> {
  ...
  try {
    ...
    const { data: booking, error: bErr } = await supabase.from('bookings').select('*').eq('booking_reference', code).single();
    if (bErr || !booking) return DEMO_JOURNEY;
    let itinerary = null;
    if (booking.itinerary_id) {
      const { data: itin } = await supabase.from('itineraries').select('*').eq('id', booking.itinerary_id).single();
      itinerary = itin;
    }
    return mapToJourney(booking, itinerary);
  } catch { return DEMO_JOURNEY; }
}
...
export function totals(j: Journey, cur: Currency = 'GBP', vehTotal = 0, repriceDelta = 0) {
  const p = j.price[cur];
  const base = p.acc + p.fly + vehTotal + repriceDelta;
  const paid = 0; // bookings table tracks this separately
  return { total: base, paid, balance: base - paid, fly: p.fly, acc: p.acc + vehTotal };
}
export function balanceDue(j: Journey): string {
  const d = new Date(j.startISO);
  d.setDate(d.getDate() - 30);
  return d.toISOString();
}
export function journeyTimeline(j: Journey) {
  const items: any[] = [];
  ...
  items.push(transferToItem(j.transfers[0] || j.homeward));
  j.segments.forEach((s, i) => {
    items.push({ type: 'segment', segment: s });
...
const legD = (kind: Leg['kind'], carrier: string, no: string, depApt: string, dep: string, arrApt: string, arr: string, status: LegStatus, cross = false): Leg =>
...
export const DEMO_JOURNEY: Journey = {
  ...
  occasion: 'honeymoon',
  themes: ['Big Five', 'water & mokoro', 'adventure', 'wine & culture'],
  price: {
