...
export default function JourneyMiniSite({ journey, mode = 'quote', visitCount = 1 }:
  ...
  return <div className="tse-ms" style={{ background: C.bg, color: C.text, fontFamily: F.b, fontWeight: 300, minHeight: '100vh' }}>
    ...
    {modal === 'share' && <ModalShell title="Invite / email a copy" onClose={() => setModal(null)}><input readOnly value={`thesafariedition.com/journey/${j.ref}`} style={input} /><div style={{ marginTop: 12 }}><Btn full onClick={() => setModal(null)}>Send</Btn></div></ModalShell>}
  </div>;
}
