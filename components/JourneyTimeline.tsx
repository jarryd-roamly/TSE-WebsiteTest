'use client';
// components/JourneyTimeline.tsx — luxe, day-anchored Selection / Review page.
// GRAFTED: presentation unchanged (scoped .tcj). Data now comes from the single
// source — app/lib/journey.ts — via the toTimelineItems() adapter. No private
// Itinerary type, no embedded DEMO.
//
//   import JourneyTimeline from '@/components/JourneyTimeline';
//   const journey = await getJourney(params.code);          // app/lib/journey
//   <JourneyTimeline journey={journey}
//     onMakeChanges={() => router.push(`/builder?edit=${journey.ref}`)}
//     onReserve={() => router.push(`/checkout?ref=${journey.ref}`)} />

import { useState } from 'react';
import { Journey, Currency, getJourneySync, toTimelineItems } from '@/app/lib/journey';
...
const badgeIcon = (t: string) => t === 'visa' ? I.passport : t === 'family' ? I.family : I.shield;
const prepIcon = (i: number) => [I.doc, I.globe, I.shield, I.bag][i % 4];

// ── Region scenes (unchanged illustrated placeholders) ───────────────────────
function Scene({ name }: { name: string }) {
  ...
  if (name === 'madikwe') return (
    <svg className="scene" viewBox="0 0 760 192" preserveAspectRatio="xMidYMid slice" aria-label="Bushveld">
...
export default function JourneyTimeline({
  journey = getJourneySync(), onMakeChanges = () => {}, onReserve = () => {},
}: { journey?: Journey; onMakeChanges?: () => void; onReserve?: () => void }) {
  const [cur, setCur] = useState<Currency>('GBP');
  const c = journey.price[cur];
  const money = (n: number) => c.sym + Math.round(n).toLocaleString('en-GB');
  const total = c.acc + c.fly, deposit = c.fly + c.acc * 0.3;
  const items = toTimelineItems(journey);                       // ← single source, via adapter
  const tint: Record<string, string> = { cape: 'var(--cape)', madikwe: 'var(--madikwe)', falls: 'var(--falls)', delta: 'var(--delta)' };
  ...
  return (
    <div className="tcj">
      ...
      <header className="hero wrap">
        ...
        <h1 className="hero-title serif">{journey.title}</h1>
        <div className="route">{journey.route.map((r, i) => (<span key={r}>{i > 0 && <i>◆</i>}{r}</span>))}</div>
        <div className="rule" />
        <div className="hero-meta"><span><b>{journey.nights}</b> nights</span><span className="dot">•</span><span><b>{journey.dates}</b></span><span className="dot">•</span><span>departs in <b>{journey.departIn}</b> days</span></div>
      ...
      <div className="wrap">
        ...
        <div className="timeline">
          {items.map((it: any, idx: number) => it.kind === 'stop' ? (
            <section className="chapter" key={idx} style={{ animationDelay: `${0.05 + idx * 0.06}s` }}>
              ...
              <div className="card">
                ...
                <div className="body">
                  ...
                  <div className="detail">{it.detail.map((d: string, i: number) => (<span key={i}>{i === 0 ? <b>{d}</b> : <>{i > 0 && <span className="sep">·</span>}{d}</>}</span>))}</div>
                  <div className="badges">{it.badges.map((b: any, i: number) => (<span className={`badge ${b.type === 'visa' ? 'visa' : b.type === 'care' ? 'care' : 'free'}`} key={i}>{badgeIcon(b.type)}{b.label}</span>))}</div>
                  <div className="acts">{it.acts.map((a: string) => (<span className="chip" key={a}>{a}</span>))}</div>
                </div>
              </div>
            </section>
          ) : (
            <div className="transfer" key={idx} style={{ animationDelay: `${0.05 + idx * 0.06}s` }}>
              ...
              {it.items.map((leg: any, j: number) => leg.type === 'flight' ? (
                ...
                <div className="leg" key={j}><span className="ic">{leg.route.includes('road') || leg.route.includes('transfer') ? I.car : I.plane}</span><div><div className="l-route">{leg.route}</div><div className="l-dur">{leg.dur}</div></div></div>
              ) : (
        ...
        <div className="panel">
          ...
          <div className="inc-grid">{journey.included.map(x => (<div className="inc-item" key={x}>{I.check}{x}</div>))}</div>
        ...
        <div className="panel" style={{ marginTop: '18px' }}>
          ...
          <div className="inc-grid">{journey.prep.map((p, i) => (<div className="inc-item" key={i}>{prepIcon(i)}<div><b>{p.title}</b> — {p.body}</div></div>))}</div>
        </div>

        <div className="ornament">{I.seal}</div>

        <div className="specialist">
          <div className="avatar serif">{journey.specialist.initials}</div>
          <div><div className="sp-name serif">{journey.specialist.name}</div><div className="sp-role">{journey.specialist.role}</div><p className="sp-rec">{journey.specialist.rec}</p></div>
        </div>
...
// ── Scoped styles (unchanged — everything under .tcj) ────────────────────────
const CSS = `
